...
name=$(su -c "cat /sys/devices/platform/soc/*.ufshc/string_descriptors/product_name")
echo "UFS设备品牌 - $name"
ufs_a=$(su -c "cat /sys/devices/platform/soc/*.ufshc/health_descriptor/life_time_estimation_a" | cut -f1 -d ' ')
echo "A - $(ufs $ufs_a)"
ufs_b=$(su -c "cat /sys/devices/platform/soc/*.ufshc/health_descriptor/life_time_estimation_b" | cut -f1 -d ' ')
echo "B - $(ufs $ufs_b)"
